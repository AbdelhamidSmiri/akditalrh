<div class="hotels view">
	...
	<div class="col-md-12 little-title-section">
		<span class="little-title"><?php echo $hotel['Hotel']['nom'] ?></span>
		<div class="actions">
		</div>
	</div>
		...
		<tbody>
			<?php foreach ($hotelprices as $hotelprice): ?>
				<tr>
					...
					<td><?php echo $hotelprice['created']; ?></td>
					<td class="actions">
						<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $hotelprice['id'])); ?>
						/
						<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $hotelprice['id']), array('confirm' => __('Are you sure you want to delete # %s?', $hotelprice['id']))); ?>
					</td>
				</tr>
			<?php endforeach; ?>
		</tbody>
